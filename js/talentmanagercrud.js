async function getTalentmanagerById() {
    ...
    try {
        let response = await fetch(`http://localhost:8080/api/talentmanager/${talentmanagerId}`)
...
async function postTalentmanager(talentmanager) {
    ...
    try {
        await fetch(`http://localhost:8080/api/talentmanager`, {
            method: "POST",
            headers: {
...
function dummyTalentmanager() {
    let talentmanager = { // placeholder data
        naam: "Jan",
		wachtwoord: "abc123",
