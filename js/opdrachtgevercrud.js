async function getOpdrachtgeverById() {
    ...
    try {
        let response = await fetch(`http://localhost:8080/api/opdrachtgever/${opdrachtgeverId}`)
...
async function postOpdrachtgever(opdrachtgever) {
    ...
    try {
        await fetch(`http://localhost:8080/api/opdrachtgever`, {
            method: "POST",
            headers: {
...
function dummyOpdrachtgever() {
    let opdrachtgever = { // placeholder data
        naam: "Youngcapital",
        wachtwoord: "abc123",
